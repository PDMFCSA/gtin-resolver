...
const { getAvailableLanguagesForBatch } = require("./utils");
...
function getMetadata(server){
    ...
    async function getDSUMetadataHandler(request, response) {
        ...
        try {
            ...
            let langs;
            try {
                const batchSSI = GTIN_SSI.createGTIN_SSI(epiDomain, undefined, gtin, batchNumber)

                const resolver = require("opendsu").loadAPI("resolver");
                const dsu = await $$.promisify(resolver.loadDSU)(batchSSI);
                langs = await getAvailableLanguagesForBatch(batchSSI)
            } catch (error) {
                logger.info(0x103, `No batch SSI created for batch: ${batchNumber}. If batch is undefined, this log is expected`);
            }

            if(langs.length > 0 && documentsMetadata && documentsMetadata.leaflet && documentsMetadata.leaflet.unspecified){
                documentsMetadata.leaflet.unspecified = documentsMetadata.leaflet.unspecified.map((el) => {
                    if(langs.includes(el.value))
                        el.batch = true;
