...
function getActivateRelatedFixedURLHandler(getReplicasFnc){
  return function activateRelatedFixedUrl(dsu, domain, gtin, batchNumber,language, callback){
    ...
    let next = async ()=>{
      ...
      getReplicasFnc(domain, function(err, replicas){
        ...
        // let filter = `gtin=${gtin}`
        // if(batchNumber){
        //   filter = `batch=${batchNumber}&`+filter
        // }
        // if(language){
        ...
        if(batchNumber){
          ...
          filter = `^(?!.*batch).*gtin=${gtin}.*`
        }
        if(language){
          filter = filter+`lang=${language}.*`
        }
        call(targets, `url like (${filter})`, callback);
      });
    }
...
function getDeactivateRelatedFixedURLHandler(getReplicasFnc){
  return function deactivateRelatedFixedUrl(dsu, domain, gtin,language, batchNumber, callback){
    getReplicasFnc(domain, function(err, replicas){
      if(replicas.length === 0){
