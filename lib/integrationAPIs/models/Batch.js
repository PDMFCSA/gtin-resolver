...
function Batch(enclave, domain, subdomain, gtin, batchNumber, version) {
    ...
    instance.persist = async (auditContext,language) => {
        ...
        try{
            ...
            $$.promisify(require("../../mappings/utils").activateGtinOwnerFixedUrl)(undefined, domain, gtin,batchNumber,language);
            $$.promisify(require("../../mappings/utils").activateLeafletFixedUrl)(undefined, domain, gtin,batchNumber,language);
        }catch(err){
            //ignore them for now...
        }
