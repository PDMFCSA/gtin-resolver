...
const createMetadataFixedUrl = async (walletDBEnclave, sourceTableName, domain, subdomain) => {
    ...
    const getAllRecords = async (tableName, limit = 250) => {
        ...
        while (hasMore) {
            try {
                const result = await dbService.filter(tableName, ["__timestamp > 0"], [{["id"]: "asc"}], limit, lastKey);
    ...
    let dbName = ["db", "db", domain.replaceAll(".", "-"), subdomain.replaceAll(".", "-"), sourceTableName].join("_")
    try {
